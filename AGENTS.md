...
The macOS Cocoa port lives in `fusepb/` and has its own `Fuse.xcodeproj`; it
uses `fusepb/Makefile` to regenerate Perl-derived source before opening Xcode.
...
- C, targeting C89 compatibility (no C99/C11 features without a configure
  guard). Objective-C is used only in the `fusepb/` Cocoa port.
...
Build command (run from `fusepb/`):

```sh
xcodebuild clean -project Fuse.xcodeproj -target Fuse -configuration Default
xcodebuild       -project Fuse.xcodeproj -target Fuse -configuration Default
```

Always run `clean` before a fresh build — `xcodebuild` incremental mode can
report stale failures from a previous broken build.
...
The project's `HEADER_SEARCH_PATHS` includes both `$(SRCROOT)/..` (fuse root)
and `$(PROJECT_DIR)` (fusepb/), so regular compilation finds them. The Clang
Static Analyzer (`AnalyzeShallow`) historically failed on C files because it
could not resolve `libspectrum.h` at analysis time. The fix is to keep
`RUN_CLANG_STATIC_ANALYZER = NO` in all build configurations in

...
HIDCreateOpenDeviceInterface(io_object_t hidDevice, recDevice * pDevice)
{
    ...
    if (NULL != pDevice->interface) {
        ...
        if (kIOReturnSuccess != result)
            if (kIOReturnExclusiveAccess != result) {
                HIDReportErrorNum
...
HIDTopLevelElementHandler(const void *value, void *parameter)
{
    ...
    if (!CFNumberGetValue
        ...
        ui_error( UI_ERROR_ERROR, "CFNumberGetValue error retrieving pDevice->usagePage.");
    refCF = CFDictionaryGetValue(value, CFSTR(kIOHIDElementUsageKey));
    if (!CFNumberGetValue
        (refCF, kCFNumberLongType, &((recDevice *) parameter)->usage))
        ui_error( UI_ERROR_ERROR, "CFNumberGetValue error retrieving pDevice->usage.");
}
                 ...
                 recDevice * pDevice)
{
    ...
    if ((KERN_SUCCESS ==
        ...
        if (kIOReturnSuccess != IOObjectRelease(parent2))
            ui_error( UI_ERROR_ERROR, "IOObjectRelease error with parent2.");
        if (kIOReturnSuccess != IOObjectRelease(parent1))
            ui_error( UI_ERROR_ERROR, "IOObjectRelease error with parent1.");
    }
}
...
SDL_SYS_JoystickInit(void)
{
    ...
    if (gpDeviceList) {
        ui_error( UI_ERROR_ERROR, "Joystick: Device list already inited.");
        return -1;
    }

    result = IOMasterPort(bootstrap_port, &masterPort);
    if (kIOReturnSuccess != result) {
        ui_error( UI_ERROR_ERROR, "Joystick: IOMasterPort error with bootstrap_port.");
        return -1;
    }
